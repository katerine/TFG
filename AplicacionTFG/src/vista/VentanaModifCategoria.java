...
public class VentanaModifCategoria  extends JFrame implements ActionListener {
	...
	public VentanaModifCategoria() {
		...
		labelTitulo.setText("Configurar Categoria\r\n");
		...
		textTitulo.setBounds(80, 156, 190, 25);
		getContentPane().add(textTitulo);
		
		textTema=new JTextField();
		textTema.setBounds(80, 191, 190, 25);
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		{
			try {
				Categorias miUsuario=new Categorias();
				miUsuario.setIdTema(Integer.parseInt(textId.getText()));
				miUsuario.setTitulo(textTitulo.getText());
				miUsuario.setTitulo(textTema.getText());

				miControla.modificarCategoria(miUsuario);
				
				if (Logica.modificaUsuario==true) {
					habilita(true, true,  true, true, false, true, true);	
				}
			} catch (Exception e2) {
		...
		{
			Categorias miPersona=miControla.buscarCategoria(textId.getText());
			if (miPersona!=null)
			{
	...
	private void muestraUsuario(Categorias usuario) {
		textTitulo.setText(usuario.getTitulo());
	 ...
	 * @param bEliminar
	 */
	public void habilita(boolean Id, boolean nombre, boolean tema,	 boolean bBuscar, boolean bGuardar, boolean bModificar, boolean bEliminar)
	{
		textId.setEditable(Id);
		textTitulo.setEditable(nombre);
		textTema.setEditable(tema);
